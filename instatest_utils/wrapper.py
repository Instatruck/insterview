...
class Wrapper(object):
    ...
    def get_session(environment, duration_seconds=86400):
        ...
        with Wrapper._locks['all']:
            ...
            if environment.get_account_number() not in Wrapper._locks:
                ...
                Wrapper._locks[environment.get_account_number(
                ) + '-role-session-lock'] = Lock()
        ...
        with Wrapper._locks['all']:
            ...
            if environment.get_role():
                ...
                if session is None:
                    session = Wrapper._get_user_session(
                        environment, duration_seconds)
                ...
                with Wrapper._locks[environment.get_account_number() + '-role-session-lock']:
                    ...
                    session = Wrapper._get_session_for_assumed_role(
                        environment, session)

            else:

                session = Session(region_name=environment.region, profile_name=os.environ.get(
                    'Wrapper_PROFILE', 'default'))
    ...
    def _get_user_session(environment, duration_seconds):
        ...
        if not Wrapper._console_available:
            raise Exception(
                'Not authenticated to Wrapper. Run authenticate command.')
    ...
    def _get_session_for_assumed_role(environment, session, role=None):
        ...
        if role is None:
            role = Wrapper._get_user_role(
                environment) or environment.get_role()

        log.info('Assuming role ' + 'arn:aws:iam::' +
                 environment.get_account_number() + ':role/' + role.strip())
        response = session.client('sts').assume_role(
            RoleArn='arn:aws:iam::' + environment.get_account_number() + ':role/' +
            role.strip(),
            RoleSessionName=''.join(random.choice(
        ...
        Wrapper._role_session_cache[environment.get_account_number()] = (
            session, data)
        ...
        if environment.get_role():
            ...
            if session_key in Wrapper._role_session_cache:
                ...
                if expiration > datetime.datetime.now(expiration.tzinfo):
                    log.debug('used cached role session for ' +
    ...
    def get_mfa_serial(aws_profile=None, aws_config_file='~/.aws/config'):
        ...
        if aws_profile:
            ...
            if not config.has_section(section):
                raise 'Requested profile ' + aws_profile + \
                    ' does not exist in ' + aws_config_file
    ...
    def _set_user_role(environment, session):
        ...
        for role in environment.get_roles():
            try:
                ...
                result = Wrapper._get_session_for_assumed_role(
        ...
        log.debug(
            'Did not find any roles you can use for environment '
            + environment.get_name()
        )
        return session
    ...
    def _save_user_role(environment, role):
        file_name = os.path.expanduser(
            '~/insterview/' + environment.get_account_number() + '.role')
